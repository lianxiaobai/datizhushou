...
public class Utils {
    ...
    //new version 头号英雄
    public Information getInformation(String str,int i) {
        //先去除空行
        str = str.replaceAll("((\r\n)|\n)[\\s\t ]*(\\1)+", "$1").
                replaceAll("^((\r\n)|\n)", "");
        str=str.replace(" ","");

        String[] questionAndAnswers = str.trim().split("\n");
        String question = "";
        List<String> answers_list = new ArrayList<>();
        int index=questionAndAnswers.length-1;
        for (int j = 0;j < 3; j++,index--) {
            answers_list.add(questionAndAnswers[index]);
        }
        //第12题有4个选项
        if(questionAndAnswers[0].indexOf("12")>-1){
            answers_list.add(questionAndAnswers[index]);
            index--;
        }
        for(int j = 0;j < index+1;j++){
            question+=questionAndAnswers[j];
        }
      /*  //问号统一替换为英文问号防止报错
        str=str.replace("？","?");
        ...
        String remain = str.substring(str.indexOf("?") + 1);
        String[] ans = remain.trim().split("\n");*/
        return new Information(question, answers_list.toArray(new String[answers_list.size()]));
    }
     ...
     * @return 返回排序的rank
     */
/*    public static int[] rank(int[] floats) {
        int[] rank = new int[floats.length];
        I[] f = Arrays.copyOf(Integer, Integer.length);
        Arrays.sort(f);
